...
# --- Compile a list of special characters ---#
spec_chars = re.compile('[@`_!#$%^&*()<>?=+/|}{~:]')
# --- Compile a list of lower case alphabets ---#
l_alphabets = re.compile('[a-z]')
# --- Compile a list of upper case alphabets ---#
u_alphabets = re.compile('[A-Z]')
# --- Compile a list of numbers ---#
nums = re.compile('[0-9]')


def detect_character(pass_string, character_set):
    """function to search strings for a given list of characters
    """
    # --- If the character isn't in the string, return false ---#
    if(character_set.search(pass_string) is None):
        res = False
    # --- If the character is in the string, return false ---#
    else:
        res = True
    return(res)
...
def check(
    password, min_length: int = 8, max_length: int = 200,
    special_chars: bool = True, numbers: bool = True, upper_case: bool = True,
    lower_case: bool = True, common_password: bool = True
):
    # --- Check if the length of the password matches the criteria set ---#
    if len(password) < min_length or len(password) > max_length:
        # --- if not, then return the criteria ---#
        return ('password must be shorter than {} characters and'
                'longer than {} characters.'.format(max_length, min_length))
    ...
    # --- Check for lower case alphabets if the parameter is true ---#w
